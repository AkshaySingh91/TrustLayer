...
if not os.path.exists("./models/summarizer/pytorch_model.bin") or not os.path.exists(
    "./models/risk_analyzer/pytorch_model.bin"
):
    raise RuntimeError(
        "❌ Models not found! Please run your model download script first."
    )

print("🔹 Loading summarizer and risk analyzer models from local storage...")
summarizer = pipeline(
    "summarization", model="./models/summarizer", tokenizer="./models/summarizer"
)
classifier = pipeline(
    "zero-shot-classification",
    model="./models/risk_analyzer",
    tokenizer="./models/risk_analyzer",
)
...
def extract_text_from_url(url: str) -> str:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Unable to fetch or parse URL: {e}"
        )
    ...
    try:
        ...
        for chunk in chunks:
            try:
                ...
                if token_count > 512:
                    print(
                        f"Warning: Chunk still too long ({token_count} tokens), truncating..."
                    )
                    # Truncate the chunk to fit
                    words = chunk.split()
                    truncated_words = words[
                        : int(len(words) * 0.7)
                    ]  # Take 70% of words as safety
...
def create_structured_summary(summary: str, original_text: str) -> dict:
    ...
    return {
        ...
        "structured_analysis": build_user_structured_output(
            original_text, enhanced_categories
        ),
                ...
                important_clauses.append(
                    ...
                    "clause": context,
                        ...
                        "concern_level": (
                            ...
                            if any(
                                word in pattern
                                for word in ["arbitration", "non-refundable", "liable"]
                            )
                            else "medium"
                        ),
                    }
...
def analyze_comprehensive_categories(text: str) -> dict:
    ...
    try:
        ...
        print(
            f"✅ Comprehensive analysis completed with {len(comprehensive_analysis)} categories"
        )
        ...
        return {
            "summary_key_points": {
                "main_purpose": "Analysis temporarily unavailable",
                "key_highlights": [],
                "document_type": "T&C",
            },
            "data_collected": {
            ...
            "liabilities": {
                "liability_clauses": {},
                "user_protection_level": "Unknown",
                "legal_jurisdiction": "Not specified",
            },
            "automatic_renewals": {
                "renewal_practices": {},
                "cancellation_difficulty": "Unknown",
                "refund_policy": {
                    "available": "unknown",
            ...
            "termination_clauses": {
                "termination_rights": {},
                "user_notice_period": "Not specified",
...
def build_user_structured_output(original_text: str, enhanced: dict) -> dict:
    """Builds the user-requested structured JSON object from our enhanced analysis.
    If any section is missing/unclear, include it with default placeholders per requirements.
    """
    try:
        # Summary points
        skp = []
        try:
            skp_main = enhanced.get("summary_key_points", {}).get("main_purpose")
            if skp_main:
                skp.append(skp_main)
            for p in enhanced.get("summary_key_points", {}).get("key_highlights", [])[
                :4
            ]:
        ...
        # Data collected
        dc_desc = "Data not present in current page."
        dc_items: list = []
        try:
            types = enhanced.get("data_collected", {}).get("types_collected", {})
            if types:
                dc_items = [
                    k.replace("_", " ")
                ...
                if dc_items:
                    dc_desc = "Detected data collection categories."
        except Exception:
            pass

        # Data usage
        du_desc = "Data not present in current page."
        du_examples: list = []
        try:
            purposes = enhanced.get("data_usage", {}).get("purposes", {})
            if purposes:
                du_examples = [
                    k.replace("_", " ")
                    for k, v in purposes.items()
                    if isinstance(v, dict) and v.get("mentioned")
                ]
                if du_examples:
                    du_desc = "Data usage purposes mentioned in the policy."
        except Exception:
            pass

        # Liabilities
        liab_desc = "Data not present in current page."
        liab_clauses: list = []
        try:
            liab = enhanced.get("liabilities", {}).get("liability_clauses", {})
            present = [
                k.replace("_", " ")
            ...
            if present:
                liab_desc = "Liability-related clauses detected."
                liab_clauses = present
        except Exception:
            pass

        # Automatic renewals
        ar_desc = "Data not present in current page."
        ar_clauses: list = []
        try:
            renewals = enhanced.get("automatic_renewals", {}).get(
                "renewal_practices", {}
            )
            mentioned = [
                k.replace("_", " ")
            ...
            if mentioned:
                ar_desc = "Subscription renewal-related clauses detected."
                ar_clauses = mentioned
        except Exception:
            pass

        # Termination clauses
        tc_desc = "Data not present in current page."
        tc_clauses: list = []
        try:
            terms = enhanced.get("termination_clauses", {}).get(
                "termination_rights", {}
            )
            present = [
                k.replace("_", " ")
            ...
            if present:
                tc_desc = "Termination-related clauses detected."
                tc_clauses = present
        except Exception:
            pass
        ...
        user_rights_risk = 0
        try:
            r = enhanced.get("risk_score_breakdown", {})
            overall_risk = int(round(float(r.get("overall_score", 0))))
            cs = r.get("category_scores", {})
            privacy_risk = int(round(float(cs.get("privacy_risk", 0))))
            # Map legal risk to liability_risk as approximation
            liability_risk = int(round(float(cs.get("legal_risk", 0))))
            # Transparency and user_rights not explicitly modeled; keep 0 unless we infer
            # Optionally infer user_rights_risk from lack of rights
            ur = enhanced.get("user_rights", {}).get("available_rights", {})
            if ur:
                available_count = sum(
                    1 for v in ur.values() if isinstance(v, dict) and v.get("available")
                )
                total = len(ur)
                # Higher risk when fewer rights are available
                if total > 0:
                    user_rights_risk = int(round((1 - (available_count / total)) * 100))
        except Exception:
            pass

        risk_summary = "Risk assessment derived from detected categories and clauses."

        return {
            "summary_key_points": skp if skp else [],
            "data_collected": {"description": dc_desc, "items": dc_items},
            "data_usage": {"description": du_desc, "examples": du_examples},
            "liabilities": {"description": liab_desc, "clauses": liab_clauses},
            "automatic_renewals": {"description": ar_desc, "clauses": ar_clauses},
            "termination_clauses": {"description": tc_desc, "clauses": tc_clauses},
            "risk_score_breakdown": {
                ...
                "categories": {
                    ...
                    "user_rights_risk": user_rights_risk,
                },
                "summary": risk_summary,
            },
        }
    except Exception:
        # Return fully defaulted structure on error
        return {
            "summary_key_points": [],
            "data_collected": {
                "description": "Data not present in current page.",
                "items": [],
            },
            "data_usage": {
                "description": "Data not present in current page.",
...
def extract_auto_summary_points(text: str) -> dict:
    ...
    summary_points = [
        sentence[2].strip() for sentence in top_sentences if sentence[0] > 0
    ]
    ...
    return {
        "main_purpose": (
            ...
            if summary_points
            else "Terms and Conditions governing service usage."
        ),
...
def find_opt_out_mechanisms(text: str) -> list:
    ...
    if "unsubscribe" in text:
        mechanisms.append("Unsubscribe")
    if "opt-out" in text or "opt out" in text:
        mechanisms.append("Opt-out")
    if "settings" in text:
        mechanisms.append("Account Settings")
    if "contact" in text:
        mechanisms.append("Contact Support")
    return mechanisms
...
def find_contact_methods(text: str) -> list:
    ...
    if "mail" in text and "address" in text:
        methods.append("Mail")
    if "form" in text:
        methods.append("Online Form")
    return methods


def find_response_timeframes(text: str) -> str:
    timeframes = re.findall(
        r"(\d+)\s*(day|week|month|business day)", text, re.IGNORECASE
    )
    if timeframes:
...
def get_usage_risk_level(purpose: str) -> str:
    ...
    return (
        "high"
        if purpose in high_risk
        else "medium" if purpose in medium_risk else "low"
    )
...
def get_sharing_risk_level(entity: str) -> str:
    ...
    return (
        "high" if entity in high_risk else "medium" if entity in medium_risk else "low"
    )
...
def calculate_user_protection(liability_clauses: dict) -> str:
    protection_count = sum(
        1 for clause in liability_clauses.values() if not clause["present"]
    )
...
def find_legal_jurisdiction(text: str) -> str:
    jurisdictions = re.findall(
        r"(law.*of|jurisdiction.*of|governed.*by).*?([A-Z][a-z]+)", text, re.IGNORECASE
    )
...
def assess_cancellation_difficulty(text: str) -> str:
    ...
    difficulty_count = sum(
        1 for indicator in difficulty_indicators if indicator in text.lower()
    )

    if difficulty_count >= 3:
        return "High"
    elif difficulty_count >= 1:
        return "Medium"
    else:
        return "Low"
...
def find_user_notice_requirements(text: str) -> str:
    notice_patterns = re.findall(
        r"(\d+)\s*(day|week|month).*notice", text, re.IGNORECASE
    )
    return (
        f"{notice_patterns[0][0]} {notice_patterns[0][1]}s"
        if notice_patterns
        else "Not specified"
    )
...
def categorize_content(text: str) -> dict:
    ...
    for category in categories:
        categories[category] = (
            min(100, int((categories[category] / max_score) * 100))
            if max_score > 0
            else 0
        )
...
def translate_comprehensive_analysis(summary_data: dict, target_language: str) -> dict:
    ...
    try:
        ...
        summary_data["executive_summary"] = translate_text(
            summary_data["executive_summary"], target_language
        )
        summary_data["key_points"] = [
            translate_text(point, target_language)
...
def analyze_risk(text: str):
    ...
    try:
        ...
        if token_count > 512:
            print(
                f"Text too long for risk analysis ({token_count} tokens), truncating..."
            )
        ...
        risk_score = (
            sum(s for l, s in scores.items() if l != "low risk / consumer friendly")
            * 12.5
        )
        ...
        return {
            ...
            "risk_level": (
                ...
                if risk_score >= 75
                    ...
                    "MEDIUM"
                    if risk_score >= 40
                    else "LOW" if risk_score >= 15 else "VERY LOW"
                )
            ),
        }
    except Exception as e:
    ...
    try:
        ...
        return {
            ...
            "metadata": {
                ...
                "summary_compression": (
                    len(summary_data["executive_summary"]) / len(data.text)
                    if len(data.text) > 0
                    else 0
                ),
            },
    ...
    try:
        ...
        return {
            ...
            "metadata": {
                ...
                "summary_compression": (
                    len(summary_data["executive_summary"]) / len(text)
                    if len(text) > 0
                    else 0
                ),
            },
