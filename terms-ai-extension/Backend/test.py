...
def extract_text_from_url(url):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Unable to fetch or parse the URL: {e}"
        )
...
def analyze_risk(text):
    ...
    risk_score = (
        sum(s for l, s in scores.items() if l != "low risk / consumer friendly") * 12.5
    )
