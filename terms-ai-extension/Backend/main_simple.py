...
def categorize_content(content: str) -> Dict[str, float]:
    ...
    categories["privacy_data"] = (
        sum(content_lower.count(term) for term in privacy_terms) * 5
    )
    ...
    categories["payments_billing"] = (
        sum(content_lower.count(term) for term in payment_terms) * 8
    )
    ...
    categories["user_obligations"] = (
        sum(content_lower.count(term) for term in obligation_terms) * 6
    )

    # Company Rights
    rights_terms = ["right", "license", "grant", "authorize", "permit", "reserve"]
    categories["company_rights"] = (
        sum(content_lower.count(term) for term in rights_terms) * 4
    )
    ...
    categories["dispute_resolution"] = (
        sum(content_lower.count(term) for term in dispute_terms) * 10
    )

    # Cookies and Tracking
    cookie_terms = ["cookie", "tracking", "analytics", "advertising", "pixel", "beacon"]
    categories["cookies_tracking"] = (
        sum(content_lower.count(term) for term in cookie_terms) * 7
    )
...
def build_structured_output(
    ...
    """Produce the structured JSON required by the extension UI.
    Falls back per spec when data is not present.
    """
    text_lower = content.lower()

    # summary_key_points
    summary_key_points = key_points[:5] if key_points else []

    # data_collected
    dc_items: List[str] = []
    if any(term in text_lower for term in ["personal data", "personal information", "pii", "name", "email", "address"]):
        dc_items.append("Personal data")
    if any(term in text_lower for term in ["usage", "activity", "behavior"]):
        dc_items.append("Usage data")
    if any(term in text_lower for term in ["cookie", "tracking", "pixel", "beacon"]):
        dc_items.append("Cookies")
    data_collected = {
        "description": (
            "Detected data collection categories." if dc_items else "Data not present in current page."
        ),
        "items": dc_items,
    }

    # data_usage
    du_examples: List[str] = []
    if any(term in text_lower for term in ["personaliz", "customiz", "recommend"]):
        du_examples.append("Personalization")
    if any(term in text_lower for term in ["market", "advertis", "promo"]):
        du_examples.append("Marketing")
    if any(term in text_lower for term in ["analytic", "analysis", "statistics", "tracking"]):
        du_examples.append("Analytics")
    data_usage = {
        "description": (
            "Data usage purposes mentioned in the policy." if du_examples else "Data not present in current page."
        ),
        "examples": du_examples,
    }

    # liabilities
    liab_clauses = [
        c.get("type") for c in important_clauses if c.get("type") == "Liability Limitation"
    ]
    liabilities = {
        "description": (
            "Liability-related clauses detected." if liab_clauses else "Data not present in current page."
        ),
        "clauses": liab_clauses,
    }

    # automatic renewals
    ar_clauses = [
        c.get("type") for c in important_clauses if c.get("type") == "Automatic Renewal"
    ]
    automatic_renewals = {
        "description": (
            "Subscription renewal-related clauses detected." if ar_clauses else "Data not present in current page."
        ),
        "clauses": ar_clauses,
    }
    ...
    # termination clauses
    term_clauses = [
        c.get("type") for c in important_clauses if c.get("type") == "Termination Rights"
    ]
    termination_clauses = {
        "description": (
            "Termination-related clauses detected." if term_clauses else "Data not present in current page."
        ),
        "clauses": term_clauses,
    }

    # risk breakdown
    privacy_risk = int(round(categories.get("privacy_data", 0)))
    liability_risk = 80 if liabilities["clauses"] else 0
    transparency_risk = 0  # not inferred in simple mode
    user_rights_risk = 0    # not inferred in simple mode
    risk_score_breakdown = {
        "overall_risk": int(round(risk.overall_risk)),
        "categories": {
        ...
        "summary": "Risk assessment derived from detected patterns and categories.",
    }
