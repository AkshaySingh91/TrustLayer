...
(function() {
    ...
    class TCDetector {
        ...
        // Auto-detect a T&C or Privacy link on the current site and trigger background URL analysis
        async autoDetectAndAnalyze() {
            try {
                // Avoid running on chrome:// or extension pages
                ...
                const candidates = links
                    ...
                    .filter(href => {
                        ...
                        return /\b(terms|privacy|policy|tos|legal)\b/.test(lower) && href.startsWith(location.origin);
                    });

                // Prefer most explicit targets
                const priority = [
                    /\/terms(\/|$)/i,
                    /\/terms-of-service(\/|$)/i,
                    /\/privacy(\/|$)/i,
                ...
                for (const p of priority) {
                    target = candidates.find(u => p.test(u));
                    if (target) break;
                }
                ...
                // Ask background to analyze URL directly
                chrome.runtime.sendMessage({
                    action: 'auto_detect_tc_link',
                    tcUrl: target
                }, (response) => {
                    // No UI toast; background will set badge/notification
                    // Silently ignore errors to avoid bothering the user while browsing
                });
            } catch (e) {
                // Silent fail
            }
