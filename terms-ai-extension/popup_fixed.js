...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadSavedLanguage();
    // Do not auto-load stale recent analysis; load for active tab instead
    // loadRecentAnalysis();
    try { chrome.runtime.sendMessage({ action: 'clear_badge' }); } catch (e) {}
    
    // Load latest analysis for the active tab/domain if available
    loadAnalysisForActiveTab();
    ...
    async function loadAnalysisForActiveTab() {
        try {
            const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
            if (!tab || !tab.url) return;
            const response = await sendMessage({ action: 'get_analysis_data', url: tab.url });
    ...
    async function checkBackendStatus() {
        ...
        try {
            ...
            console.log("object", response.success)
    ...
    function updateStatusItem(element, status) {
        ...
        const statusItemParent = element.parentElement; 
        const colors = {
            connected: '#28a745',
            disconnected: '#dc3545',
            checking: '#ffc107'
        };
        statusItemParent.style.borderLeftColor = colors[status] || '#6c757d';
    }
    ...
    function displayResults(data) {
        ...
        // Display source/analyzed URL
        if (analyzedUrlEl) {
            const src = data.source_url || data.source_url === '' ? data.source_url : (data.source_url ?? data?.metadata?.source_url);
            analyzedUrlEl.textContent = `Analyzed page: ${src || 'Current page'}`;
        }
        ...
        if (data.summary) {
            ...
            // Prefer structured_analysis → enhanced_analysis → nothing (no fallback demo)
            const structured = data.summary.structured_analysis;
            if (structured) {
                displayStructuredAnalysis(structured);
            } else if (data.summary.enhanced_analysis) {
                displayComprehensiveAnalysis(data.summary.enhanced_analysis);
            } else {
                console.log('⚠️ No structured/enhanced analysis found; skipping demo fallback');
            }
        }
    ...
    function displayStructuredAnalysis(structured) {
        console.log('🔍 Displaying structured analysis:', structured);
        try {
            // Summary / Key Points
            if (structured.summary_key_points) {
                const summaryTextEl = document.getElementById('enhanced-summary-text');
                const summaryGrid = document.getElementById('enhanced-summary-grid');
                if (summaryTextEl) {
                    const first = structured.summary_key_points[0] || 'Summary not available';
                    summaryTextEl.textContent = first;
                }
                if (summaryGrid) {
                    const rest = (structured.summary_key_points || []).slice(1);
                    if (rest.length) {
                        summaryGrid.innerHTML = rest.map(p => `
                            <div class="summary-card">
                                <div class="summary-card-icon">•</div>
                                <div class="summary-card-title">Key Point</div>
                                <div class="summary-card-value">${escapeHtml(p)}</div>
                            </div>
                        `).join('');
                    } else {
                        summaryGrid.innerHTML = '<div class="no-data">No additional key points</div>';
                    }
                }
            }

            // Data Collected
            const dc = structured.data_collected;
            if (dc) {
                    ...
                    if (!items.length) {
                        ...
                        container.innerHTML = items.map((name, idx) => `
                            <div class="chart-bar">
                                ...
                                <div class="chart-bar-fill"><div class="chart-fill-inner" style="width: ${Math.min(100, 20 + (idx%4)*20)}%"></div></div>
                                <div class="chart-value">${Math.min(100, 20 + (idx%4)*20)}%</div>
                            </div>
                        `).join('');
                    }
                }
            ...
            // Data Sharing → map to wordcloud from liabilities/data_sharing is not present in structured; skip

            // User Rights not present in structured; skip

            // Liabilities
            const liab = structured.liabilities;
            if (liab) {
                const container = document.getElementById('liabilities-text');
                if (container) {
                    const clauses = liab.clauses || [];
                    if (!clauses.length) {
                        container.innerHTML = '<div class="no-data">No liability information available</div>';
                    } else {
                        container.innerHTML = clauses.map(c => `<span class="text-danger">${escapeHtml(c)}</span> `).join('');
                    }
                }
            }

            // Automatic Renewals
            const ar = structured.automatic_renewals;
            if (ar) {
                ...
                if (container) {
                    ...
                    if (!clauses.length) {
                        ...
                        container.innerHTML = clauses.map((name, idx) => `
                            <div class="chart-bar">
                                <div class="chart-label">${escapeHtml(name)}</div>
                                <div class="chart-bar-fill"><div class="chart-fill-inner" style="width: ${Math.min(100, 25 + (idx%3)*25)}%"></div></div>
                                <div class="chart-value">${Math.min(100, 25 + (idx%3)*25)}%</div>
                            </div>
                        `).join('');
                    }
                }
            ...
            // Termination
            const term = structured.termination_clauses;
            if (term) {
                const container = document.getElementById('termination-timeline');
                if (container) {
                    const clauses = term.clauses || [];
                    if (!clauses.length) {
                        ...
                        container.innerHTML = clauses.map(c => `
                            <div class="timeline-item" style="border-left-color:#f59e0b">
                                ...
                                <div class="timeline-content">
                                    <div class="timeline-title">${escapeHtml(c)}</div>
                                    <div class="timeline-description">Present</div>
                                </div>
                            </div>
                        `).join('');
                    }
            ...
            const rs = structured.risk_score_breakdown;
            if (rs && rs.categories) {
                const heatmapContainer = document.getElementById('risk-breakdown-heatmap');
                if (heatmapContainer) {
                    const entries = Object.entries(rs.categories);
                    if (!entries.length) {
                        heatmapContainer.innerHTML = '<div class="no-data">No risk breakdown available</div>';
                    } else {
                        heatmapContainer.innerHTML = entries.map(([name, score]) => {
                            const s = Math.round(Number(score) || 0);
                            const riskClass = s >= 75 ? 'heatmap-high' : (s >= 50 ? 'heatmap-medium' : 'heatmap-low');
                            return `
                                <div class="heatmap-cell ${riskClass}">
                                    <div>${escapeHtml(name.replace(/_/g,' '))}</div>
                                    <div>${s}%</div>
                                </div>`;
                        }).join('');
                    }
                }
                // Radar chart stays as-is, using existing createRadarChart if desired
            }
        } catch (e) {
            console.error('displayStructuredAnalysis error', e);
        }
    }
